...
def data_loader():
    ...
    clear_plots_params(ax_LambdaMetr, "r", "LambdaMetr", 12, 10 )
    clear_plots_params(ax_PhiMetr, "r", "PhiMetr", 12, 10 )
...
def update_Metr(frame, ax_LambdaMetr, ax_PhiMetr):
    ...
    my_plotting(ax_LambdaMetr,to_be_ploted[0], to_be_ploted[4], plot_R, plot_phiScal_inf, colors[c_i])
    my_plotting(ax_PhiMetr,to_be_ploted[0], to_be_ploted[6], plot_R, plot_phiScal_inf, colors[c_i])
...
if __name__ == "__main__":
    ...
    global ax_LambdaMetr, ax_PhiMetr

    fig_Metr, axs = get_figure(2)
    ax_LambdaMetr= axs[0]
    ax_PhiMetr = axs[1]

    ani_phiscal = FuncAnimation(
      fig = fig_Metr, func = update_Metr,
      fargs = ( ax_LambdaMetr, ax_PhiMetr),
      frames = data_loader,
      interval = 500
    )
